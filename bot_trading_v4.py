...
def run_strategy(symbol, lock):
    ...
    while not STOP_EVENT.is_set():
        try:
            ...
            # Señales siempre sobre vela CERRADA (-2) para evitar lookahead.
            POSITION_SIZE_SIMPLE = 0.03  # 3% del balance disponible
            trailing_sl_atr_mult_fixed = 1.0

            data = get_data_binance(symbol, interval=timeframe, limit=260)
            df = cal_metrics_technig(data, 14, 10, 20)

            # Asegurar indicadores necesarios aunque cal_metrics_technig cambie.
            if 'ema200' not in df.columns:
                df['ema200'] = ta.trend.EMAIndicator(df['close'], window=200).ema_indicator()
            if 'atr' not in df.columns:
                atr_ind = ta.volatility.AverageTrueRange(high=df['high'], low=df['low'], close=df['close'], window=ATR_WINDOW)
                df['atr'] = atr_ind.average_true_range()

            # Highest high 10 (sin incluir la vela de señal)
            df['highest_high_10'] = df['high'].rolling(window=10).max().shift(1)
            ...
            logger.info(
                f"[{symbol}] Precio: {close_price:.4f} | Vol: {volatility:.2f}% | "
                f"EMA200: {float(ema200):.4f} | HH10(prev): {float(highest_high_10):.4f} | Posiciones: {len(positions)}"
            )

            # Position sizing (sin régimen)
            position_size_active = float(POSITION_SIZE_SIMPLE)
            ...
            # Entrada (Breakout simple):
            # Operar SOLO si close > EMA200 y close rompe el máximo de las últimas 10 velas (sin incluir la vela de señal)
            entry_ok = (float(close_price) > float(ema200)) and (float(close_price) > float(highest_high_10))
            ...
            if entry_ok and balance > symbol_min_notional and len(positions) == 0:
                ...
                if qty is None:
                    ...
                    atr_entry = float(atr)
                    stop_loss = entry_price - (1.5 * atr_entry)
                    ...
                    if resp:
                        ...
                        new_position = {
                            ...
                            'trailing_sl_atr_mult': float(trailing_sl_atr_mult_fixed),
                            'trailing_active': True,
